...
/** Ensayo aislado de 0021 sobre un export D1 en 0020; nunca toca el origen. */
...
function verify(db, phase) {
  ...
  if (scalar(db, 'SELECT count(*) AS value FROM inventory_counts') !== 0) throw new Error(`${phase}: la migración inventó conteos`);
}

const args = argsOf(process.argv.slice(2));
const baseline = resolve(args.baseline);
const directory = join(resolve(args.output), `r3-inventory-counts-${Date.now()}`);
...
if (!/CREATE TABLE (?:IF NOT EXISTS )?[`"]?orders\b/i.test(sql)) {
  for (const name of readdirSync(resolve('migrations')).filter((name) => /^00(?:0[1-9]|1[0-9]|20)_.*\.sql$/.test(name)).sort()) {
...
if (!exists(db, 'inventory_transfers') || exists(db, 'inventory_counts')) throw new Error('baseline debe estar en 0020');
const beforeHash = hash(snapshot(db));
db.exec(readFileSync(resolve('migrations/0021_inventory_counts.sql'), 'utf8'));
verify(db, 'forward');

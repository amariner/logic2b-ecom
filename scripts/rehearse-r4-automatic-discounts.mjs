...
/** Ensayo aislado de 0027 sobre un export D1 en 0026; nunca toca el origen. */
...
const AUTOMATIC_TABLES = [
  'automatic_discounts', 'automatic_discount_products', 'automatic_discount_applications',
];
...
function verify(db, phase) {
  integrity(db, phase);
  for (const table of AUTOMATIC_TABLES) {
  ...
  for (const trigger of [
    'automatic_discount_application_insert_guard',
    'promotion_code_usage_automatic_conflict_guard',
  ]) {
    if (!exists(db, 'trigger', trigger)) throw new Error(`${phase}: falta ${trigger}`);
  }
  for (const index of [
    'idx_automatic_discounts_active',
    'idx_automatic_discount_products_product',
    'idx_automatic_discount_applications_discount',
  ]) {
...
const directory = join(resolve(args.output), `r4-automatic-discounts-${Date.now()}`);
...
if (!exists(db, 'table', 'promotion_code_usages') || exists(db, 'table', 'automatic_discounts')) {
  throw new Error('baseline debe estar en 0026');
}
const before = snapshot(db);
const beforeHash = hash(before);
db.exec(readFileSync(resolve('migrations/0027_automatic_discounts.sql'), 'utf8'));
verify(db, 'forward');
...
const summary = {
  ...
  promotions: before.promotions.length,
  promotionUsages: before.promotionUsages.length,
  beforeHash,
};
db.close();

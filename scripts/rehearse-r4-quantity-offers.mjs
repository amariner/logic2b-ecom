...
/** Ensayo aislado de 0028 sobre un export D1 en 0027; nunca toca el origen. */
...
const TABLES = [
  'quantity_offers', 'quantity_offer_tiers', 'quantity_offer_products', 'quantity_offer_applications',
];
...
function verify(db, phase) {
  ...
  for (const trigger of [
    'quantity_offer_tier_insert_guard',
    'quantity_offer_product_insert_guard',
    'quantity_offer_application_insert_guard',
    'promotion_code_usage_quantity_offer_conflict_guard',
    'automatic_discount_application_quantity_offer_conflict_guard',
  ]) {
    if (!exists(db, 'trigger', trigger)) throw new Error(`${phase}: falta ${trigger}`);
  }
  for (const index of [
    'idx_quantity_offers_active',
    'idx_quantity_offer_products_product',
    'idx_quantity_offer_applications_offer',
  ]) {
...
const directory = join(resolve(args.output), `r4-quantity-offers-${Date.now()}`);
...
if (!exists(db, 'table', 'automatic_discounts') || exists(db, 'table', 'quantity_offers')) {
  throw new Error('baseline debe estar en 0027');
}
const before = snapshot(db);
const beforeHash = hash(before);
db.exec(readFileSync(resolve('migrations/0028_quantity_offers.sql'), 'utf8'));
verify(db, 'forward');

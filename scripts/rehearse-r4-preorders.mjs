...
/** Ensayo aislado de 0033 sobre un export D1 en 0032; nunca toca el origen. */
...
const TABLES = ['preorder_policies', 'preorder_commitments',
  'preorder_commitment_events', 'preorder_allocations'];
...
function snapshot(db) {
  return {
    ...
    storedValue: db.prepare('SELECT * FROM stored_value_accounts ORDER BY id').all(),
  };
}
...
function verify(db, phase) {
  ...
  for (const trigger of ['preorder_policy_activation_insert_guard',
    'preorder_policy_activation_update_guard', 'preorder_commitment_insert_guard',
    'preorder_commitment_event_guard', 'preorder_allocation_insert_guard']) {
    if (!exists(db, 'trigger', trigger)) throw new Error(`${phase}: falta ${trigger}`);
  }
  for (const index of ['idx_preorder_policies_state', 'idx_preorder_commitments_order',
    'idx_preorder_commitments_fifo', 'idx_preorder_commitment_events_commitment',
    'idx_preorder_allocations_commitment']) {
...
const directory = join(resolve(args.output), `r4-preorders-${Date.now()}`);
...
if (!exists(db, 'table', 'stored_value_accounts') || exists(db, 'table', 'preorder_policies')) {
  throw new Error('baseline debe estar en 0032');
}
const before = snapshot(db);
const beforeHash = hash(before);
db.exec(readFileSync(resolve('migrations/0033_preorders_backorders.sql'), 'utf8'));
verify(db, 'forward');

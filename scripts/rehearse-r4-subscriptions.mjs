...
/** Ensayo aislado de 0034 sobre un export D1 en 0033; nunca toca el origen. */
...
const TABLES = ['subscription_plans', 'subscriptions', 'subscription_provider_events',
  'subscription_events', 'subscription_cycles'];
...
function snapshot(db) {
  return {
    ...
    preorders: db.prepare('SELECT * FROM preorder_commitments ORDER BY id').all(),
  };
}
...
function verify(db, phase) {
  ...
  for (const trigger of ['subscription_insert_guard', 'subscription_event_insert_guard',
    'subscription_cycle_insert_guard']) {
    if (!exists(db, 'trigger', trigger)) throw new Error(`${phase}: falta ${trigger}`);
  }
  for (const index of ['idx_subscription_plans_variant', 'idx_subscriptions_status',
    'idx_subscription_provider_events_subscription', 'idx_subscription_events_subscription',
...
const directory = join(resolve(args.output), `r4-subscriptions-${Date.now()}`);
...
if (!exists(db, 'table', 'preorder_policies') || exists(db, 'table', 'subscription_plans')) {
  throw new Error('baseline debe estar en 0033');
}
const before = snapshot(db);
const beforeHash = hash(before);
db.exec(readFileSync(resolve('migrations/0034_provider_subscriptions.sql'), 'utf8'));
verify(db, 'forward');

...
/** Ensayo aislado de 0038 sobre un export D1 en 0037; nunca toca el origen. */
...
function verify(db, phase) {
  integrity(db, phase);
  for (const table of ['customer_data_rights_evidence',
    ...
    if (scalar(db, `SELECT count(*) AS value FROM ${table}`) !== 0) {
      throw new Error(`${phase}: 0038 inventó evidencia`);
    }
  }
  for (const trigger of ['customer_data_rights_evidence_insert_guard',
    'customer_data_rights_evidence_update_guard',
    'customer_data_rights_plan_decision_insert_guard',
    'customer_data_rights_plan_decision_update_guard',
    'customer_data_rights_artifact_reference_insert_guard',
    'customer_data_rights_artifact_reference_update_guard']) {
    if (!exists(db, 'trigger', trigger)) throw new Error(`${phase}: falta ${trigger}`);
  }
  for (const index of ['idx_customer_data_rights_request_version',
    'idx_customer_data_rights_request_history', 'idx_customer_data_rights_profile']) {
...
const directory = join(resolve(args.output), `r5-data-rights-evidence-${Date.now()}`);
...
if (!exists(db, 'table', 'customer_profiles') ||
    ...
    exists(db, 'table', 'customer_data_rights_evidence')) {
  throw new Error('baseline debe estar exactamente en 0037');
}
const before = snapshot(db);
const beforeHash = hash(before);
db.exec(readFileSync(resolve('migrations/0038_data_rights_evidence.sql'), 'utf8'));
verify(db, 'forward');
if (hash(snapshot(db)) !== beforeHash) throw new Error('0038 alteró datos existentes');

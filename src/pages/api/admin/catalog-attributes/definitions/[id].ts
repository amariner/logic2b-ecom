...
const schema = z.object({
  ...
  unit: z.string().trim().min(1).max(24).nullable(), constraints: z.record(z.unknown()), active: z.boolean(),
}).strict();
...
export const PATCH: APIRoute = async ({ params, request, locals }) => {
  ...
  try { return catalogAdminMutationResponse(await createAdminOperations(locals.runtime.env.DB).updateAttributeDefinition(id, parsed.data)); }
  catch (error) { return catalogAdminErrorResponse(error) ?? Promise.reject(error); }
};
export const DELETE: APIRoute = async ({ params, locals }) => {
  const denied = gate(locals); if (denied) return denied;
  const id = Number(params.id); if (!Number.isInteger(id) || id <= 0) return Response.json({ error: 'id inválido' }, { status: 400 });
  try { return catalogAdminMutationResponse(await createAdminOperations(locals.runtime.env.DB).deleteAttributeDefinition(id)); }
  catch (error) { return catalogAdminErrorResponse(error) ?? Promise.reject(error); }
};

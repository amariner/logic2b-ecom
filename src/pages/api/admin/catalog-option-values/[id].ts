...
export const PATCH: APIRoute = async ({ params, request, locals }) => {
  ...
  try {
    return catalogAdminMutationResponse(
      await createAdminOperations(locals.runtime.env.DB).updateProductOptionValue(checked, parsed.data),
    );
  } catch (error) {
...
export const DELETE: APIRoute = async ({ params, locals }) => {
  ...
  try {
    return catalogAdminMutationResponse(
      await createAdminOperations(locals.runtime.env.DB).deleteProductOptionValue(checked),
    );
  } catch (error) {
    const response = catalogAdminErrorResponse(error);
    if (response) return response;

...
export const POST: APIRoute = async ({ params, request, locals }) => {
  ...
  try {
    const result = await createInventoryCountOperations(env.DB).submit(params.id, parsed.data.expected_version, parsed.data.idempotency_key);
    ...
    return Response.json({ error: error instanceof Error ? error.message : 'No se pudo aplicar el conteo.' }, { status: 409 });
  }
};

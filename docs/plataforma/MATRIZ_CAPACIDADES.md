...
| CUS-006 | Direcciones guardadas | módulo | P2 | parcial | ADR-0044, D1 local `0042` y backup 35 materializan selector `addr_` estable, owner desde revisión vigente, lectura owner-only, append CAS transaccional, merge fail-closed y purga coherente. Permanece instalada sin flags; HTTP/UI, CSRF, idempotencia de comando y rollout siguen pendientes. |

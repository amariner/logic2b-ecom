...
| CUS-003 | Cuentas sin contraseña | módulo | P2 | parcial | ADR-0042/0043, D1 `0039`–`0040` y backup 33 implementan identidad separada, Resend directo con tracking verificado, `prepare→persist→deliver`, confirmación durable previa al consumo, cookie real/dummy del mismo navegador, fragmento→POST, HTTP/UI, throttle, auditoría, revocación y gate CAS fail-closed. Permanece `installed`: D1/secretos/entrega real y activación requieren rollout aislado; sin WebAuthn, step-up, cross-device ni ownership R5.5. |
| CUS-004 | Historial y seguimiento de pedidos | módulo | P2 | parcial | ADR-0044, D1 `0041`, reader opaco y backup 34 instalan referencia aleatoria/versionada y owner canónico sin reclamar historia guest. Permanece instalado e inactivo: authorizer HTTP, ruta/UI, navegación y rollout siguen pendientes. |
| CUS-005 | Autoservicio de devolución | módulo | P2 | especificado | ADR-0044 separa lectura/creación y obliga a revalidar el owner del pedido en la transacción; FUL-010, persistencia y UI siguen pendientes. |
| CUS-006 | Direcciones guardadas | módulo | P2 | especificado | ADR-0044 separa lectura/escritura sobre revisiones propias con CAS; persistencia de referencia pública, policy y superficie siguen pendientes. |

...
| CUS-005 | Autoservicio de devolución | módulo | P2 | parcial | Selector `ret_`, owner/CAS desde `ord_`, elegibilidad transaccional, replay/carrera y HTTP/SSR local están implementados tras sesión, scope, CSRF y rate limit; falta cerrar navegador/a11y y rollout. |
...
| CUS-009 | Segmentos calculados | módulo | P2 | especificado | ADR-0045 y contrato R5.6a: facts ausentes `null`, lenguaje limitado, templates/instancias versionados e inmutables, parámetros exactos y lifecycle observable fail-closed. Persistencia, job, rutas y UI esperan gates propios. |

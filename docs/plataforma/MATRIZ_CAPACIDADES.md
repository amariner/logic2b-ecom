...
| CUS-006 | Direcciones guardadas | módulo | P2 | parcial | ADR-0044, `0042`–`0043` y backup 36 cubren selector `addr_`, owner/revisión vigente, índice, alta y revisión con CSRF, idempotencia durable y CAS transaccional. API y `/cuenta/direcciones` SSR pasan IDOR, replay, carrera y a11y en fixture local inerte. Permanece instalada sin flags: la D1 persistente local sigue en `0042`, producción en `0040` y el rollout por proyecto continúa pendiente. |

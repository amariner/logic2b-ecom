...
| 13 | Plataforma modular: del gestor mínimo a paridad extrema de capacidad | 🟡 En curso | 2026-08-21 | **R0–R3.12, R4, R5.1–R5.4d y R5.5a–d completos; R4.9–R5.5d conservan rollout propio:** D1 producción permanece en `0040`; `0041`, API e interfaz owner-only están verificadas solo en local, con `CUS-004 installed` e inactivo. Siguiente R5.5e, referencias y ownership de direcciones guardadas, sin activar ni desplegar. Fuente de verdad en [`docs/plataforma/`](plataforma/README.md). |
...
R5.5d completa localmente el recorrido de lectura de `CUS-004`. El índice
`GET /api/customer/orders` deriva el perfil solo de la sesión, pagina diez
pedidos con cursor opaco y rechaza parámetros alternativos; la consulta filtra
owner y perfil activo dentro de D1. `/cuenta/pedidos` y su detalle son SSR, sin
JavaScript, con navegación condicionada, vacío/error, estados y tracking. API,
índice y detalle comparten rate limit por IP/superficie, headers privados y
telemetría sin referencia ni PII.

El arnés local activa una composición inerte sin DB, cookies, secretos ni red y
prueba HTTP real más 18 superficies a 1440/375 con 0 errores y 0 avisos; la
revisión visual confirma cero overflow. `customers@1.9.0`, `pnpm check` con 190
suites/993 tests y 756 archivos tipados, build y E2E global quedan verdes. No
hubo DDL, D1 remota, Worker, proveedor, activación ni deploy; la demo conserva
API y páginas en 404 antes del runtime.

Continuación exacta: R5.5e debe instalar `CUS-006` en el registro/preset avanzado
y materializar referencias `addr_` opacas para las direcciones versionadas de
R5.1. La referencia representa el `address_id` estable; la revisión vigente y
el perfil activo son el ownership canónico. El bloque debe definir versionado
CAS para lectura/escritura, backfill sin exponer dirección/contacto, reader D1,
backup y rehearsal/restore, colisiones/concurrencia y perfil fusionado. No abre
HTTP/UI ni activa la capacidad; cualquier migración remota o deploy conserva un
preflight y autorización separados.
